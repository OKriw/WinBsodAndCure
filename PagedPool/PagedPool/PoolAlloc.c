...
NTSTATUS DriverEntry(__in PDRIVER_OBJECT pDriverObject, __in PUNICODE_STRING path)
{
	...
	if (!NT_SUCCESS (Status))
	{
		IoDeleteDevice (pFakeDevice);
		return Status;
	}
	KdPrint(("Driver entry is finished\n"));
	KBSOD(); //Done to stay in system thread
	return Status;
};
...
VOID RaiseIrql()
{
	KIRQL OldIrql;
	NTSTATUS Status;
	PUCHAR TryToAcessPool;
	CONTEXT TestContext;

	OldIrql=KeGetCurrentIrql();
	KeRaiseIrql(DISPATCH_LEVEL, &OldIrql);

	TryToAcessPool=(PUCHAR)PoolPtr;
	KeLowerIrql(OldIrql);
};
...
NTSTATUS PoolAlloc()
{
	...
	PHYSICAL_ADDRESS BytesToSkip;
	SIZE_T TotalBytes = 4096;
	LowAddress.QuadPart = 0,
	HighAddress.QuadPart = (ULONGLONG)-1;
	BytesToSkip.QuadPart = 0;

	PoolPtr = MmAllocatePagesForMdlEx(LowAddress,
									HighAddress,
									BytesToSkip,
									TotalBytes,
									1,
	...
	AddressPoolptr = MmMapLockedPagesSpecifyCache(PoolPtr,
												KernelMode,
												1,
												NULL,
												FALSE,
	...
	Status = MmProtectMdlSystemAddress(PoolPtr, PAGE_READONLY);
	return Status;
}
...
{
	SIZE_T TotalBytes = 4096;
	PUCHAR SomeTrash = "This is statment to be put in memory";
	SpoilContext();
...
VOID KBSOD()
{
	...
	KIRQL OldIrqlDpc;
	PVOID Check;
	SIZE_T PagedBufferSize=256;
	UNICODE_STRING KeBugCheckExAdress;
	UNICODE_STRING KeBugCheck2Adress;
	UINT64 opcodes0;
	UINT64 opcodes1; //This is used for the correct patching KeBugCheckEx

	RtlInitUnicodeString(&KeBugCheckExAdress, L"KeBugCheckEx");
	...
	KeLowerIrql(OldIrql); 
	KdPrint(("Now we have adresses before BSOD!"));
};
...
{	
	...
	for (Frame = 0; Frame < 12; Frame++) {
		RuntimeFunction = RtlLookupFunctionEntry(GlobalContext.Rip,
												&ImageBase,
												&UnwindHistoryTable);
		...
		if(Frame == 3) {
			...
			Status = MmProtectMdlSystemAddress(PoolPtr,PAGE_READWRITE);
		}
		if(Frame == 4) {
			Irql = KeGetCurrentIrql();
